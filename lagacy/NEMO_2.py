    ...
    def cluster_detect(self):
        ...
        for num_points in range(2, ind_weak.size + 1):
            ...
            #    continue
                ...
                print(ind_weak_work.size)
                print(ind_cluster_work.size)
                while not np.array_equal(np.sort(ind_cluster_work), np.sort(ind_weak_work)):
                    print("#####")
                    #print(self._work_obs.data().as_numpy_array()[ind_weak_work])
                    #print(self._work_obs.data().as_numpy_array()[ind_cluster_work])
                    #print(self._work_obs.d_spacings().data().as_numpy_array()[ind_weak_work])
                    #print(self._work_obs.d_spacings().data().as_numpy_array()[ind_cluster_work])

                    if ind_weak_work.size > ind_cluster_work.size:
                        j_work, i_work, ind_weak_work = self.remove_idx(j_work, i_work, ind_weak_work)
                    elif ind_weak_work.size < ind_cluster_work.size:
